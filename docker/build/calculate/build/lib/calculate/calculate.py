...
class CalculateNode(Node):
    def __init__(self):
        ...
        self.tracked_bbox = None    # [x1, y1, x2, y2, ...]
    ...
    def yolo_callback(self, msg):
        self.tracked_bbox = msg.data  # [id, x1, y1, x2, y2, ...]
        self.try_calculate()
        

    def try_calculate(self):
        if self.drone_info is None or self.tracked_bbox is None:
            return
        if len(self.tracked_bbox) < 5:
            return  # 沒偵測到任何框

        x1, y1, x2, y2 = self.tracked_bbox[1:5]
        cx = (x1 + x2) / 2
        cy = (y1 + y2) / 2
